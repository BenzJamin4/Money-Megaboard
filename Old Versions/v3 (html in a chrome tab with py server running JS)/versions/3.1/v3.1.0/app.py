    ...
    if rules:
        for i in range(len(transactions)):
            ...
            for j in range(len(transactions)):
                ...
                # Date logic
                # JS stored dates as strings after parsing, so we expect ISO strings here to parse back
                try:
                    ...
                    continue # Skip if unparseable dates
                ...
                for rule in rules:
                    ...
                    matchA = (rule['acc1'] in t1['account'].lower() and rule['desc1'] in t1['desc'].lower() and
                              rule['acc2'] in t2['account'].lower() and rule['desc2'] in t2['desc'].lower())
                    matchB = (rule['acc1'] in t2['account'].lower() and rule['desc1'] in t2['desc'].lower() and
                              rule['acc2'] in t1['account'].lower() and rule['desc2'] in t1['desc'].lower())
                    
                    if matchA or matchB:
                        rule_matched = True
                        break
                        
                if rule_matched or (days_apart < 1 and t1['account'] != t2['account']):
                    t1['isTransfer'] = True
                    t2['isTransfer'] = True
...
def process_data():
    ...
    for group in groups:
        ...
        for file_data in group.get('filesData', []):
            ...
            rows = file_data.get('rows', []) # raw CSV row arrays from JS basically
            ...
            for i in range(1, len(rows)):
                ...
                if not note_val and n_idx != -1 and len(cols) > n_idx:
                    note_val = cols[n_idx]
...
def upload_paypal():
    ...
    clean_export = clean_df.drop(columns=['Timestamp', 'Abs_Amount', 'Status_Debug'], errors='ignore')
