    ...
    for i in range(len(transactions)):
        ...
        if not matched_pair:
            ...
            for rule in rules:
                ...
                if rule['acc1'] in t1_acc and rule['desc1'] in t1_desc:
                    if rule['acc2'] not in existing_accounts:
                        t1['isTransfer'] = True
                        t1['category'] = "Transfers"
                        t1['transferPartnerTxId'] = "ghost"
                        t1['desc'] = f"{t1['desc']} (Ghost: {rule['acc2']})"
                        break
                        
                # Match B: t1 matches acc2 side, acc1 is missing
                elif rule['acc2'] in t1_acc and rule['desc2'] in t1_desc:
                    if rule['acc1'] not in existing_accounts:
                        t1['isTransfer'] = True
                        t1['category'] = "Transfers"
                        t1['transferPartnerTxId'] = "ghost"
                        t1['desc'] = f"{t1['desc']} (Ghost: {rule['acc1']})"
                        break
