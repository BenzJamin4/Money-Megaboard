// app.js - Frontend Logic for Money Megaboard v3.3.1
...
async function downloadGhostCSV() {
    ...
    // Also include manually-isolated ghost transfers
    allTransactions.forEach(tx => {
        if (tx.isolate && tx.category === 'Transfers' && tx.manualTransferAccount) {
            const dateStr = tx.date.toLocaleDateString('en-US');
...
function renderTransferRules() {
    ...
    appSettings.transferRules.forEach(rule => {
        ...
        if (rule.acc1 && !acc1IsActive) ghostAccounts.add(rule.acc1);
        if (rule.acc2 && !acc2IsActive) ghostAccounts.add(rule.acc2);
    });
...
// Track currently selected account so it persists across re-renders
let _selectedBalanceAccount = null;
...
async function renderBalanceColors() {
    ...
    const renderSwatches = () => {
        ...
        _selectedBalanceAccount = selectedAcc;
    ...
    // Restore previously selected account if it still exists, otherwise default to first
    if (_selectedBalanceAccount && allAccounts.includes(_selectedBalanceAccount)) {
        select.value = _selectedBalanceAccount;
    } else if (allAccounts.length > 0) {
        select.value = allAccounts[0];
        _selectedBalanceAccount = allAccounts[0];
    }
    renderSwatches();
}
    ...
    allTransactions.forEach(tx => {
        ...
        if (tx.isTransfer && tx.transferPartnerTxId) {
            ...
            if (partner) {
                ...
                displayDesc = highlightKeywords(`Transfer: ${src} -> ${dst}`);
        ...
        if (tx.isolate && tx.category === 'Transfers' && !isConsolidatedTransfer) {
            // Show account dropdown for ghost transfer target selection
        ...
        } else if (tx.isolate && !isConsolidatedTransfer) {
            accountHtml = `<td><select class="acc-select" data-id="${tx.id}" style="font-weight:bold; width:100%; cursor:pointer;">
                ${allAccountsList.map(a => `<option value="${a}" ${tx.account === a ? 'selected' : ''}>${a}</option>`).join('')}
            </select></td>`;
        }
