    ...
    allTransactions.forEach(tx => {
        ...
        if (tx.isTransfer && tx.transferPartnerTxId) {
            ...
            if (partner) {
                ...
                if (timeSrc < timeDst) {
                    if (srcIsMidnight && !dstIsMidnight) finalTime = timeDst;
                    else finalTime = timeSrc;
                } else {
                    if (dstIsMidnight && !srcIsMidnight) finalTime = timeSrc;
                    else finalTime = timeDst;
                }
...
function updatePieCharts() {
    ...
    const renderPie = (canvas, data) => {
        ...
        return new Chart(canvas, { type: 'pie', data: { labels, datasets: [{ data: Object.values(data), backgroundColor: labels.map(getCategoryColor), borderColor: '#000', borderWidth: 2 }] }, options: { plugins: { legend: { position: 'bottom' } } } });
    };
