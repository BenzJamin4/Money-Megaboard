...
function updateCharts() {
    ...
    allAccountsList.forEach((acc) => {
        ...
        for (let i = 0; i < rawData.length; i++) {
            ...
            // Redundant duplicates (straight line) become null ONLY if they are exactly 0 to break the line completely 
            if (val === 0) {
                // Keep the first 0 and the last 0 of a consecutive string. 
                // Only make it null if BOTH the previous value AND the next value are also 0.
...
function updatePieCharts() {
    ...
    const renderPie = (canvas, data) => {
        ...
        return new Chart(canvas, { type: 'pie', data: { labels, datasets: [{ data: Object.values(data), backgroundColor: labels.map(getCategoryColor), borderColor: '#000', borderWidth: 2 }] }, options: { plugins: { legend: { position: 'top' } } } });
    };
