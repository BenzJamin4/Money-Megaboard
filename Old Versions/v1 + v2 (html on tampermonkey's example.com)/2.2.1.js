...
/*
    CHANGELOG:
    v2.2.1 (Bug Fixes) - Removed grey backgrounds. Grouped split CSV mappings (Name#1 + Name#2 -> Name). Kept account name case sensitivity. Fixed Applebees/Apple categorization overlap. Enlarged charts and stacked pos/neg bars below 0. Moved Notes to last column.
...
(function () {
    ...
    document.body.innerHTML = `
        <div style="max-width: 1300px; margin: 0 auto; font-family: sans-serif; padding: 20px;">
            <h2>Money Megaboard v2.2.1</h2>

            <div style="margin-bottom: 30px; padding: 15px; background: #fff; border: 1px solid #dcdde1; border-radius: 5px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 40px; margin-bottom: 40px;">
                <div style="width: 100%; background: #fff; padding: 15px; border: 1px solid #dcdde1; border-radius: 5px;">
                    <canvas id="netWorthChart" style="max-height: 400px;"></canvas>
                </div>
                <div style="width: 100%; background: #fff; padding: 15px; border: 1px solid #dcdde1; border-radius: 5px;">
                    <canvas id="posNegChart" style="max-height: 400px;"></canvas>
                </div>
            </div>
    ...
    function setupEventListeners() {
        document.getElementById("csvFiles").addEventListener("change", async e => {
            ...
            for (let file of files) {
                ...
                // Strip extension, then strip #1, #2 to group files
                ...
                if (!pendingGroups[groupKey]) {
                    pendingGroups[groupKey] = {
                        accountName: baseName, // preserves exact casing of the first file in group
                        headers: headers,
                        filesData: []
                    };
                }
    ...
    function guessCategory(desc, amount) {
        ...
        if (d.includes("wawa") || d.includes("restaurant") || d.includes("pizza") || d.includes("mcdonald") || d.includes("starbucks") || d.includes("applebee")) return "Food";
        ...
        allTransactions.forEach(tx => {
            ...
            row.innerHTML = `
                ...
                <td style="padding: 5px;">
                    <input type="text" class="note-input" data-id="${tx.id}" value="${tx.notes}" style="width:100%; padding:4px; border:1px solid #ccc; border-radius:3px; box-sizing:border-box;" placeholder="Add note...">
                </td>
            `;
    ...
    function updateCharts() {
        ...
        chronTxs.forEach(tx => {
            ...
            if (!tx.isTransfer) {
                ...
                // Add the negative amount natively so the bar plots below $0
