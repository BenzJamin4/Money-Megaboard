...
/*
    CHANGELOG:
    v2.3.0 (Major)       - Enforced pure #000000 global CSS (killed all grey in inputs/graphs). Built Transfer Rules UI for multi-day/keyword linking (fixes PayPal gaps). Cleaned Account column to hide "(group)" tags. Restored all categories to Pie Charts with vibrant colors.
    v2.2.3 (Feature/Bug) - ZERO GREY policy started. Added (tag) grouping. Removed Groceries/Investment. Advanced PayPal sanitizer (removes "Shopping Cart Item" duplicates).
    ...
    v2.0.1 (Bug Fixes)   - Fixed negative number parsing.
...
(function () {
    ...
    // Inject strict global CSS to kill all browser-default greys
    const style = document.createElement('style');
    style.innerHTML = `
        body { background-color: #ffffff !important; color: #000000 !important; }
        input, select, button { color: #000000 !important; border: 2px solid #000000 !important; background: #ffffff !important; outline: none; }
        input::placeholder { color: #000000 !important; opacity: 0.6; }
        input[type="file"]::file-selector-button { border: 2px solid #000000; background: #ffffff; color: #000000; font-weight: bold; padding: 5px 10px; cursor: pointer; }
    ...
    document.body.innerHTML = `
        <div style="max-width: 1300px; margin: 0 auto; font-family: sans-serif; padding: 20px; background-color: #ffffff; color: #000000;">
            <h2 style="color: #000000; border-bottom: 3px solid #000000; padding-bottom: 5px;">Money Megaboard v2.3.0</h2>
            ...
            <div style="margin-bottom: 30px; padding: 15px; background: #ffffff; border: 3px solid #000000;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                    ...
                    <div>
                        <button id="clearStorageBtn" style="padding: 8px 12px; background: #e74c3c; color: #ffffff !important; cursor: pointer; font-weight: bold;">
                            Clear Saved Data
                        </button>
                    </div>
                </div>
                ...
                <div id="mappingContainer" style="margin-top: 20px; display: none; background: #ffffff; padding: 15px; border: 2px dashed #000000;">
                    ...
                    <button id="processMappedDataBtn" style="margin-top: 15px; padding: 8px 15px; background: #27ae60; color: #ffffff !important; cursor: pointer; font-weight: bold;">
            ...
            <div style="margin-bottom: 30px; padding: 15px; background: #ffffff; border: 3px solid #000000;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <button id="addTransferRuleBtn" style="padding: 5px 10px; background: #000000; color: #ffffff !important; cursor: pointer; font-weight: bold;">+ Add Rule</button>
                </div>
                ...
                <button id="applyRulesBtn" style="margin-top: 15px; padding: 5px 10px; background: #2980b9; color: #ffffff !important; cursor: pointer; font-weight: bold; display: none;">Apply Rules to Current Data</button>
            </div>
            ...
            <div style="margin-bottom: 30px; padding: 15px; background: #ffffff; border: 3px solid #000000;">
                ...
                <button id="updatePieBtn" style="margin-left: 10px; padding: 5px 10px; background: #000000; color: #ffffff !important; cursor: pointer; font-weight: bold;">Update Pie Charts</button>
    ...
    script.onload = () => {
        // Enforce Chart.js Global "No Grey" Rules
        Chart.defaults.color = '#000000';
        Chart.defaults.font.family = 'sans-serif';
        Chart.defaults.font.weight = 'bold';
        Chart.defaults.borderColor = '#000000'; // Global grid/border color
        setupEventListeners();
        renderTransferRules();
    };
    ...
    function setupEventListeners() {
        document.getElementById("csvFiles").addEventListener("change", async e => {
            ...
            for (let file of files) {
                ...
                // Clean Account Name for Table (strips the "(fmfcu)" part)
    ...
    function renderTransferRules() {
        ...
        transferRules.forEach((rule, idx) => {
            ...
            div.innerHTML = `
                ...
                <button class="del-rule-btn" data-idx="${idx}" style="padding:5px; background:#e74c3c; color:#fff !important; cursor:pointer; font-weight:bold;">X</button>
            `;
    ...
    function detectTransfers(transactions) {
        ...
        // 2. Fallback Generic Same-Day Matching (Only if exact same day and opposite amounts)
        const amountMap = {};
    ...
    function getCategoryColor(cat, isUnaccountedTransfer = false) {
        ...
        const map = {
            ...
            Education: "#2980b9", Subscription: "#16a085", Transfers: "#0055ff",
            Income: "#27ae60", Dividends: "#20bf6b", Entertainment: "#8e44ad",
            "Clothes+Haircuts": "#e67e22", Health: "#ff4757", Other: "#000000"
        };
    ...
    function renderTable() {
        ...
        allTransactions.forEach(tx => {
            ...
            let catOptions = [...new Set([...INCOME_CATS, ...EXPENSE_CATS])].map(c =>
                `<option value="${c}" ${tx.category === c ? 'selected' : ''} style="color:#000000; background:#ffffff;">${c}</option>`
            ).join('');

            if (!INCOME_CATS.includes(tx.category) && !EXPENSE_CATS.includes(tx.category)) {
            ...
            row.innerHTML = `
                ...
                <td style="padding: 5px; background:${bgColor}; border: 2px solid #000000;">
                    <select class="cat-select" data-id="${tx.id}" style="width:100%; background:transparent; color:#ffffff !important; font-weight:bold; border:none !important; cursor:pointer;" ${isConsolidatedTransfer ? 'disabled' : ''}>
                        ${catOptions}
                    </select>
                </td>
        ...
        allTransactions.forEach(tx => {
            if (!tx.isTransfer && tx.date >= start && tx.date <= end && tx.category !== "Transfers") {
                if (tx.amount < 0) {
                    ...
                    // Automatically route positive values to income unless they are refunds in an expense category
