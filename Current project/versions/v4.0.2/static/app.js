...
function updateCharts() {
    ...
    const sparseZeros = (rawData) => {
        let sparse = [...rawData];
        for (let i = 0; i < rawData.length; i++) {
            if (rawData[i] === 0) {
                const prevIsNonZero = i > 0 && typeof rawData[i - 1] === 'number' && rawData[i - 1] !== 0;
                const nextIsNonZero = i < rawData.length - 1 && typeof rawData[i + 1] === 'number' && rawData[i + 1] !== 0;
                if (!prevIsNonZero && !nextIsNonZero) {
                    sparse[i] = null;
                }
            }
        }
    ...
    datasets.push({
        ...
        data: sparseZeros(labels.map(l => monthly[l].net)),
        borderColor: '#000000',
    ...
    allAccountsList.forEach((acc) => {
        ...
        let sparseData = sparseZeros(rawData);
