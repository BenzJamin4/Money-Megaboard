...
function updateCharts() {
    ...
    // Calculate Net Worth total sum of visible data points
    const netWorthSum = labels.reduce((sum, l) => sum + (monthly[l].net || 0), 0);
    const sumNode = document.getElementById("netWorthTotalSumNode");
    if (sumNode) {
        sumNode.innerText = `Visible Net Worth Sum: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(netWorthSum)}`;
    }
    ...
    // Compute monthly positive and negative sums for percentage stack scaling
    const totalPosForMonth = {};
    const totalNegForMonth = {};
    labels.forEach(l => {
        let sumPos = 0;
        let sumNeg = 0;
        allAccountsList.forEach(acc => {
            const val = monthly[l].accountTotals[acc] || 0;
            if (val > 0) sumPos += val;
            else if (val < 0) sumNeg += Math.abs(val);
        });
        totalPosForMonth[l] = sumPos;
        totalNegForMonth[l] = sumNeg;
    });

    const stackedDatasets = allAccountsList.map(acc => {
        const data = labels.map(l => {
            const val = monthly[l].accountTotals[acc] || 0;
            if (stackedChartMode === 'absolute') {
                return val;
            } else {
                if (val > 0) {
                    const totalPos = totalPosForMonth[l];
                    return totalPos > 0 ? (val / totalPos) * 100 : 0;
                } else if (val < 0) {
                    const totalNeg = totalNegForMonth[l];
                    return totalNeg > 0 ? (val / totalNeg) * -100 : 0;
                }
                return 0;
            }
        });
